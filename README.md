...
To disable client-side discovery and load balancing, for example when sending through a proxy or an external load balancer, set an explicitly empty seed host list. An explicit list always wins over the host in `endpoint_url`, so the two can be set in either order:
